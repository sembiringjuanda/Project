...
select substring(FIRST_NAME,1,3) as FIRST_3_NAME from worker;
...
select*from worker;
select*from title;
alter table title add primary key (WORKER_REF_ID);
...
INSERT INTO Bonus 
	(WORKER_REF_ID, BONUS_AMOUNT, BONUS_DATE) VALUES
		(001, 5000, '16-02-20');
SELECT*,count(BONUS_AMOUNT) FROM bonus group by BONUS_AMOUNT
having(count(BONUS_AMOUNT) > 1);


-- Q-22. Write an SQL query to clone a new table from another table.

create table new_bonus select * from bonus;
select * from new_bonus;
